...
export class LibroService{
    ...
    getLibros():Observable<any>{//para poder ver lo que hay dentro 
    ...
    guardarLibro(libro:Libro):Observable<any>{
        let params = JSON.stringify(libro); //les transformo a un json 
        let headers = new HttpHeaders().set('Content-Type','application/json'); 
    ...
    getLibro(id: String):Observable<any>{
        let headers = new HttpHeaders().set('Content-Type','application/json');
        return this._http.get(this.url+'libro/'+id, {headers:headers});//obtener un libro en específico
    }
    //editar libro 
    //http://localhost:3600/libro:id 
